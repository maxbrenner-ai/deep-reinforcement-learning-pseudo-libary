...
class Agent:
    def __init__(self, model, optimizer, policy, action_size, state_processor, gamma=0.95, target_model_update_policy='soft',
        ...
        self.replay_period_wait = replay_period_wait
    ...
    def check_update_target_model(self, step):
        if self.target_model_update_policy == 'hard':
            ...
            self.tar_model.set_weights(tau * np.asarray(self.beh_model.get_weights()) + (1 - tau) *
                                       np.asarray(self.tar_model.get_weights()))

    def train(self, env, nb_steps, nb_steps_ep_max=None, print_rew_cb=None, print_eps_cb=None, visualize=False,
              allow_printing=True):
        ...
        runner = Runner(RunType.TRAIN, self, env, nb_steps, nb_steps_ep_max_in, print_rew_cb, send_eps, None, visualize,
                        allow_printing)
        runner.run()

    def test(self, env, nb_steps, nb_steps_ep_max=None, print_rew_cb=None, benchmark=None, visualize=False,
             allow_printing=True):
        nb_steps_ep_max_in = nb_steps_ep_max
        if nb_steps_ep_max is not None:
            nb_steps_ep_max_in = nb_steps_ep_max - 1
        # The policy auto switches to greedy when testing so no epsilon cb even allowed
        runner = Runner(RunType.TEST, self, env, nb_steps, nb_steps_ep_max_in, print_rew_cb, None, benchmark, visualize,
                        allow_printing)
        runner.run()
    ...
    def summary(self):
        ...
        text += 'Replay period wait: {}\n'.format(self.replay_period_wait)
